...
during the proving process, we have a subgoal as

   \R[ Hf1 \f* Hf, Hb1 \b* Hb] ==> \R[ Hf2, Hb2]
->  (Hf1 \* Hf) =f=> Hf2 
    /\ (Hb1 \b* Hb) =b=> Hb2

actually, we cannot prove the subgoal because that
...
(*
Definition hwand' (H1 H2:hprop) : hprop :=
  \exists Hf Hb, \R[Hf,Hb] \* \[ (H1 \* \R[Hf,Hb]) ==> H2 ].
...
Lemma himpl_refine_f : forall h Hf1 Hb1 Hf2 Hb2,
  (\R[ Hf1, Hb1 ]) h -> (\R[ Hf2, Hb2]) h
->
  (Hf1 (h `f) -> Hf2 (h `f)).
Proof. introv (MA&MB) (NA&NB). auto. Qed.

Lemma himpl_refine_b : forall h Hf1 Hb1 Hf2 Hb2,
  (\R[ Hf1, Hb1 ]) h -> (\R[ Hf2, Hb2]) h
->
  (Hf1 (h `f) -> Hf2 (h `f)).
Proof. introv (MA&MB) (NA&NB). auto. Qed.
...
Parameter union_hempty_r : forall h,
  h \g h_empty = h.


Lemma hexists_inner_intro : forall F B (f:F) (b:B) Jf Jb h,
  (\R[(Jf f),(Jb b)]) h ->
  (\R[(hfexists Jf),(hbexists Jb)]) h.
Proof.
  introv M.
  destruct M as [MA MB].
  splits.
  applys* hfexists_intro f.
  applys* hbexists_intro b.
Qed.

Parameter himpl_refine : forall Hf1 Hb1 Hf2 Hb2,
  ((\R[ Hf1, Hb1]) ==> (\R[ Hf2, Hb2]))
-> (Hf1 =f=> Hf2)/\(Hb1 =b=> Hb2).

Parameter himpl_inner_pure : forall h P1 P2,
  (\R[\f[P1],\b[P2]] h) <->
  h = h_empty /\ P1 /\ P2.


Lemma hwand_inner_l : forall Hf1 Hf2 Hb1 Hb2,
  (\R[Hf1,Hb1] \- \R[Hf2,Hb2])
==> 
  \R[Hf1 \f-* Hf2, Hb1 \b-* Hb2].
Proof.
  ...
  destruct M as (h1&h2&M1&M2&M3&M4).
  rewrite hstar_sep in M2.
  applys hexists_inner_intro Hf Hb.
  rewrite <- hstar_sep. rewrite M4.
  applys* hstar_intro.
  apply hpure_inv in M2 as [M21 M22].
  apply himpl_inner_pure. split~.
  apply~ himpl_refine.
  ...
  apply hpure_inv in M2 as [M21 M22].
  subst. rewrite union_hempty_r.
  split.
  applys himpl_refine_f M1.

  exists (Hf \f* \f[Hb1 \b* Hb =b=> Hb2]).
  rewrite <- union_empty_r.
  rewrite hfstar_assoc.
  applys* hfstar_intro.
  rewrite <- union_empty_r. applys* hfstar_intro.
  applys hfpure_intro.
...
exists Hf.
  ...
  applys* hfstar_intro.
  applys hfpure_intro.
  rewrite <- hstar_sep in M21.
  apply M1.
lets E : M21 h1.
  destruct M1 as (MA&MB).
  ...
  applys hfpure_intro.
  intros hf MF.
  specializes M21 (hf,hb_empty).
  destruct M21.
  apply M21.

Lemma hwand_equiv : forall Hf Hb H1 H2,
  ((\R[Hf,Hb]) ==> H1 \-* H2) <-> ((H1 \* \R[Hf,Hb]) ==> H2).
Proof.
  unfold hwand. iff M.
  - intros h.
    intros (h1&h2&NA&NB&NC&ND).
    apply M in NB.
    destruct NB as (Hf'&Hb'&NB).
    rewrite hstar_comm in NB.
    apply hstar_hpure_iff in NB as (NB1&NB2).
    apply NB1. rewrite ND.
    applys~ hstar_intro.
...
Lemma hwand_equiv_refine : forall Hf1 Hf2 Hb1 Hb2,
  (\R[Hf1,Hb1] \-* \R[Hf2,Hb2]) ==>
  (\R[Hf1 \f-* Hf2, Hb1 \b-* Hb2]).
Proof.
  intros. intros h M.
  ...
  apply hstar_hpure_iff in M as [MA MB].
  rewrite hstar_sep in MA.
  destruct MB as (MBf&MBb).
  rewrite hstar_sep


  unfold hfwand,hbwand. splits.
  - exists Hf. 
    rewrite hfstar_comm, hfstar_hpure_iff.
    split.
    apply himpl_refine_f.
    intros hf F.
    lets A : MA h. clear MA.
    eapply himpl_refine in A as [F B].
    intros hf.
    .

Lemma hwand_equiv_refine : forall Hf0 Hf1 Hf2 Hb0 Hb1 Hb2,
  (\R[Hf0,Hb0] ==> \R[Hf1,Hb1] \-* \R[Hf2,Hb2])
<-> ((\R[Hf1,Hb1] \* \R[Hf0,Hb0]) ==> \R[Hf2,Hb2]).
Proof. intros. applys hwand_equiv. Qed.

Lemma hwand_cancel : forall H1 H2,
  H1 \* (H1 \-* H2) ==> H2.
Proof. intros. apply hwand_equiv. applys himpl_refl. Qed.

Lemma hwand_cancel_file : forall Hf1 Hf2 Hb1 Hb2 Hb3,
  \R[Hf1,Hb1] \* (\R[Hf1,Hb2] \-* \R[Hf2,Hb3]) ==> \R[Hf2,Hb1 \b* Hb2 \b* Hb3].
Proof.
  intros. intros h M.
  destruct M as 
